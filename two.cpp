...
int main2(int argc, const char **argv) {
    ...
    if (capture.isOpened()) {
        ...
        while (1) {
            ...
            //Mat schmol;
            //cv::resize(frame.clone(), schmol, cv::Size(), 0.5, 0.5);
            //cv::imwrite("./test.jpg", frame.clone());
            //cv::imwrite("./test.jpg", schmol);
...
void detectAndDraw(Mat &img, CascadeClassifier &cascade,
    ...
    for (size_t i = 0; i < faces.size(); i++) {
        ...
        center2.x = 300;
        center2.y = 200;
        circle(img, center2, 100, Scalar(0, 0, 255), 5, 8, 0);
        */
    }
    ...
    Mat schmol;
    cv::resize(img.clone(), schmol, cv::Size(), 0.5, 0.5);
    cv::imwrite("./test.jpg", schmol);
}
